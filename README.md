...
- **Production-vs-benchmark eval gap (measured).** The offline benchmark uses pure Chinese questions; the production BaZi path prepends `"BaZi analysis for someone born on {date}, gender: {sex}. "` before the user's question, which can pollute HyDE. A 10-pair controlled shadow eval ([BENCHMARK_REPORT.md §Appendix A](docs/BENCHMARK_REPORT.md#附录-aproduction-shadow-eval)) measures the impact: GPT-4o-judged retrieval quality drops from **3.5 → 3.0 (out of 5)** with the production prefix (4 control wins / 5 ties / 1 treatment win). The proposed fix — strip the English prefix before HyDE, keep it only in the final-answer prompt — is documented but not yet implemented.
